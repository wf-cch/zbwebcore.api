...
namespace zbwebcore.API.Config
{
    public class ImageHelper
    {
        ...
        static ConfigInfo.ImageSize imagesize = ConfigInfo.Info.imageSize;
        ...
        public static byte[] ImageMaxCutByCenter(System.IO.Stream fileStream, string filename)
        {
            int saveWidth = Convert.ToInt32(imagesize.width);
            int saveHeight = Convert.ToInt32(imagesize.height);
            int quality = Convert.ToInt32(imagesize.quality);
            if (saveWidth == 0) saveWidth = 1024;
            if (saveHeight == 0) saveHeight = 768;
            ...
            var sKImage2 = SkiaSharp.SKImage.FromBitmap(sKBitmap2);
            sKBitmap2.Dispose();
            var data = sKImage2.Encode(GetImageFormatBySuffix(filename), quality);
            sKImage2.Dispose();
            bytes = data.ToArray();
            data.Dispose();
        ...
        public static byte[] ImageScalingToRange(System.IO.Stream fileStream,string filename)
        {
            
            int maxWidth = Convert.ToInt32(imagesize.width);
            int maxHeight = Convert.ToInt32(imagesize.height);
            int quality = Convert.ToInt32(imagesize.quality);
        ...
        public static byte[] ImageScalingByOversized(System.IO.Stream fileStream, string filename)
        {
            int maxWidth = Convert.ToInt32(imagesize.width);
            int maxHeight = Convert.ToInt32(imagesize.height);
            int quality = Convert.ToInt32(imagesize.quality);
            ...
            var sKImage = SkiaSharp.SKImage.FromBitmap(sKBitmap);
            sKBitmap.Dispose();
            var data = sKImage.Encode(GetImageFormatBySuffix(filename), quality);
            sKImage.Dispose();
            bytes = data.ToArray();
            data.Dispose();
        ...
        /// <param name="keepWhiteBorderPixelVal">白边处理(负值表示不做处理，最大值不超过真实二维码区域的1/10)</param>
        /// <returns></returns>
        public static byte[] QRCoder(string text, SkiaSharp.SKEncodedImageFormat format= SkiaSharp.SKEncodedImageFormat.Png, byte[] logoImgae = null, int keepWhiteBorderPixelVal = -1)
        {
            byte[] reval = null;
            int width = 320;
            int height = 320;
